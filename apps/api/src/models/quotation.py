...
class Quotation(Base):
    ...
    margin_percentage = Column(Float, default=0.00)
...
class QuoteLine(Base):
    ...
    discount_percent = Column(Float, default=0.00)
    
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
    
    quotation = relationship("QuoteLine", back_populates="lines") if False else relationship("Quotation", back_populates="lines")
