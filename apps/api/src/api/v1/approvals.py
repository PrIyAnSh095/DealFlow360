...
def get_approvals(db: Session = Depends(get_db), current_user: User = Depends(RoleChecker(APPROVAL_VIEW_ROLES))):
    # 1. Fetch existing quotation IDs already in approval requests
    existing_req_quote_ids = set(r[0] for r in db.query(ApprovalRequest.quotation_id).all())
    requester_id = current_user.id if (current_user and hasattr(current_user, 'id')) else "u-sales"
    new_reqs = []

    # 2. Fetch quotes for deals in approval status in 1 query
    quotes_for_approval_deals = db.query(Quotation).join(Deal, Quotation.deal_id == Deal.id).filter(func.lower(Deal.status) == "approval").all()
    for quote in quotes_for_approval_deals:
        if quote.id not in existing_req_quote_ids:
            new_reqs.append(ApprovalRequest(
                quotation_id=quote.id,
                requester_id=requester_id,
                status="PENDING"
            ))
            existing_req_quote_ids.add(quote.id)

    # 3. Fetch quotations where requires_approval is True
    ...
    # 4. Query all approval requests with joined eager loading in 1 query
    reqs = db.query(ApprovalRequest)\
        .options(
            joinedload(ApprovalRequest.quotation).joinedload(Quotation.deal).joinedload(Deal.customer),
            joinedload(ApprovalRequest.logs)
        )\
    ...
    for req in reqs:
        ...
        if deal:
            cust_name = deal.customer_name if deal.customer_name else (
                deal.customer.name if (deal.customer and hasattr(deal.customer, 'name') and deal.customer.name)
                else (deal.customer.company if (deal.customer and hasattr(deal.customer, 'company')) else "Customer")
            )
